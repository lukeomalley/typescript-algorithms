 ...
 * example, the run "AAAAAAAAAAAA" (12 A's), can not naively be encoded as "12A",
 * since this string can be decoded as either "AAAAAAAAAAAA" or "1AA". Thus long
 * runs (runs of 10 or more characters) could be encoded in a split fashion; the
 * aforementioned run should be encoded as "9A3A".
 *
 * Input: AAAAAAABBBBBBBBB
 * Output: 5A9B
 *
 * Time: O(n)
 * Space: O(n)
...
console.log(runLengthEncoding('AAABBBBBBBBBCCCCCCCCCCCC'));
evalFunctionPerformance(runLengthEncoding, 'AAABBBBBBBBBCCCCCCCCCCCC');
